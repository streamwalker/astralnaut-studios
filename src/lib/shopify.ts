...
export const SHOPIFY_STORE_PERMANENT_DOMAIN = "astralnaut-studios-joils.myshopify.com";
export const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
export const SHOPIFY_STOREFRONT_TOKEN = "87893b3ed0afd7c28bc54f07d3a3b474";
...
function isCartNotFoundError(userErrors: UserErr[]) {
...
export async function createShopifyCart(item: CartItemBase) {
...
export async function addLineToShopifyCart(cartId: string, item: CartItemBase) {
  const data = await storefrontApiRequest(CART_LINES_ADD_MUTATION, {
    cartId,
    lines: [{ quantity: item.quantity, merchandiseId: item.variantId }],
  });
  const userErrors: UserErr[] = data?.data?.cartLinesAdd?.userErrors || [];
...
export async function updateShopifyCartLine(cartId: string, lineId: string, quantity: number) {
  ...
  const userErrors: UserErr[] = data?.data?.cartLinesUpdate?.userErrors || [];
  if (isCartNotFoundError(userErrors)) return { success: false, cartNotFound: true };
  if (userErrors.length) {
    console.error(userErrors);
    return { success: false };
...
export async function removeLineFromShopifyCart(cartId: string, lineId: string) {
  ...
  const userErrors: UserErr[] = data?.data?.cartLinesRemove?.userErrors || [];
  if (isCartNotFoundError(userErrors)) return { success: false, cartNotFound: true };
  if (userErrors.length) {
    console.error(userErrors);
    return { success: false };
