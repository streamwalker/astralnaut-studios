...
export async function listUsersImpl(input: { search?: string; page?: number }) {
  const page = input.page ?? 1;
  const perPage = 100;
  const { data: usersData, error: usersErr } = await supabaseAdmin.auth.admin.listUsers({ page, perPage });
...
export async function getUserDetailImpl(input: { userId: string }) {
  const { data: userRes, error: userErr } = await supabaseAdmin.auth.admin.getUserById(input.userId);
  ...
  for (const e of events ?? []) {
    const s = sessions.get(e.session_id) ?? { start: e.created_at, end: e.created_at, events: 0, duration_ms: 0 };
    s.events++;
...
export async function inviteUserImpl(input: { email: string }) {
  const { data: res, error } = await supabaseAdmin.auth.admin.inviteUserByEmail(input.email);
  if (error) throw new Error(error.message);
  return { id: res.user?.id ?? null };
}

export async function createUserImpl(input: { email: string; password: string; display_name?: string }) {
  const { data: res, error } = await supabaseAdmin.auth.admin.createUser({
    email: input.email,
    password: input.password,
    email_confirm: true,
    user_metadata: input.display_name ? { display_name: input.display_name } : {},
  });
  if (error) throw new Error(error.message);
  return { id: res.user?.id ?? null };
}

export async function updateUserImpl(input: { userId: string; email?: string; display_name?: string }) {
  const updates: any = {};
  if (input.email) updates.email = input.email;
  if (input.display_name !== undefined) updates.user_metadata = { display_name: input.display_name };
  const { error } = await supabaseAdmin.auth.admin.updateUserById(input.userId, updates);
  if (error) throw new Error(error.message);
  return { ok: true };
}

export async function setUserRoleImpl(input: { userId: string; grant: boolean }, callerId: string) {
  if (input.grant) {
    ...
    if (input.userId === callerId) throw new Error("You cannot revoke your own admin role.");
    const { error } = await supabaseAdmin
      .from("user_roles")
      .delete()
      .eq("user_id", input.userId)
...
export async function sendPasswordResetImpl(input: { email: string }) {
  const { error } = await supabaseAdmin.auth.admin.generateLink({ type: "recovery", email: input.email });
  if (error) throw new Error(error.message);
  return { ok: true };
}

export async function deleteUserImpl(input: { userId: string }, callerId: string) {
  if (input.userId === callerId) throw new Error("You cannot delete your own account.");
